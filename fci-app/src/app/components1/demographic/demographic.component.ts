...
export class DemographicComponent implements OnInit {
  ...
  saveDemographic(){
    ...
    for(let i=0;i<divs.length;i++){
      ...
      flag=this.answers.some(function(answer) {
        return isDisabled || (answer['response']!='' && answer['question_id'] == qId);
      });
  ...
  isChecked(option,question_id){
    if(option!=undefined && option.is_other_option=='1' && option.response!='' && option.response!=null){
        ...
        let flag= (input!=null && radio!=null && radio.checked);
        flag?(input.disabled=false,input.value=option.response):(input.value='');
        return flag;
    }
